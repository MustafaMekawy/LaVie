...
class OrderClass {
  ...
  static plcacItems = async (req, res) => {
    try {
      ...
      myhelper.reshandeler(res,200,true,{ items, amount },`click on this link http://127.0.0.1:3000/api/v1/order/orderdetails`
      );
    } catch (e) {
  ...
  static orderDetails = async (req, res) => {
    try {
      ...
      await order.save();
      myhelper.reshandeler(res,200,true,order,'order detalis Click here to conform order .');
    } catch (e) {
      myhelper.reshandeler(res, 500, false, e, e.message);
    }
  };
  ...
  static confirmOrder = async (req, res) => {
    try {
      ...
      await order.save();
      const emailMessage = `To confirm your order with this items: ${JSON.stringify(order)},
  ...
  static confirmCode = async (req, res) => {
    try {
      ...
      await order.save()
      myhelper.reshandeler(res,200,true,null,'order is created and will delevir soon');
    } catch (e) {
      myhelper.reshandeler(res, 500, false, e, e.message);
    }
  };
  static myOrders=async(req,res)=>{
    try {
      ...
      myhelper.reshandeler(res, 200, true, req.user.myOrders,"its your orders");
    } catch (e) {
      myhelper.reshandeler(res, 500, false, e, e.message);
    }
  }
