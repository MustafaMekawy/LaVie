const Plant = require("../../db/model/plant.model")
...
class PlantClass{
    static createPlant=async(req,res)=>{
        try{
            ...
            await plant.save()
            myhelper.reshandeler(res, 200, true, plant, 'plant created in your shop')
        }
        catch(e){
    ...
    static buyPlant=async(req,res)=>{
        try{
            ...
            myhelper.reshandeler(res, 200, true, req.user.orders, 'buy plant is done')
        }
        catch(e){
          myhelper.reshandeler(res, 500, false,e,e.message)
        }
