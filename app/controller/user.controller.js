...
class UserClass {
  ...
  static signUp = async (req, res) => {
    try {
      ...
      myhelper.reshandeler(res, 200, true, { user, token }, 'SignedUp succesfully, go to this link to choose your level : http://127.0.0.1:3000/api/v1/user/chooseCategory');
    } catch (e) {
  ...
  static profile = (req,res)=>{
    myhelper.reshandeler(res, 200, true,{user: req.user},"user profile fetched")
  }
  static editprofile = async(req,res) => {
    try{
        ...
        myhelper.reshandeler(res, 200, true,user, "user ediet successfully")
    }
    catch(e){
      myhelper.reshandeler(res, 500, false, e, e.message);
    }
  ...
  static changePassword=async(req,res)=>{
    try{
      ...
      if(!await User.Passcheck(req.user,password))throw new Error('invalid password')
      const newPassword=req.body.newPassword
      req.user.password=newPassword
      await req.user.save({validateBeforeSave:false})
      myhelper.reshandeler(res, 200, true, req.user, 'password change Successfully');
    }
    catch(e){
      myhelper.reshandeler(res, 500, false, e, e.message);
    }
  ...
  static resetpassword=async(req,res)=>{
    try{
      ...
      const user = await User.findOne({resetPasswordToken:deecryptedToken,resetExpiresTime: {$gt : Date.now()}})
  ...
  static chooseCategory=async(req,res)=>{
    try{
      ...
      if(!inputCategory)throw new Error("categroy must be one of [beginner,advanced,professional]")
      req.user.category=inputCategory
      await req.user.save({validateBeforeSave:false})
      myhelper.reshandeler(res, 200, true, req.user, 'category added Successfully');
    }
    catch(e){
      myhelper.reshandeler(res, 500, false, e, e.message);
    }
  ...
  myhelper.reshandeler(res, 200, true, null, 'contact us in thses numbers')
  }
